...
const ImprimirAsistenciaSesiones = () => {
  ...
  const [titulomat] = useState('REPORTE DE ASISTENCIAS POR SESIONES');
  ...
  useEffect(() => {
    ...
    const fetchDatos = async () => {
      try {
        const resultado = await obtenerAsistenciasesiones(sede, semestre, escuela, curricula, curso, seccion, 'T', '1', '01', '01', token);
  ...
  // ✅ Detectar dinámicamente el número de sesiones
  const totalSesiones = datos.length > 0 
    ? Object.keys(datos[0]).filter(k => /^s\d+_asis$/.test(k)).length 
    : 0;
  ...
  const columnasEncabezado = [
    [
      ...
      { titulo: 'SESIONES (Total de asistencias)', colSpan: totalSesiones + 1 }
    ],
    [
      ...Array.from({ length: totalSesiones }, (_, i) => ({
        titulo: String(i + 1).padStart(2, '0') // genera 01, 02, ...
      })),
      { titulo: 'Total' }
    ]
  ];
  ...
  const columnas = [
    ...
    ...Array.from({ length: totalSesiones }, (_, i) => ({
      clave: `s${String(i + 1).padStart(2, '0')}_asis`
    })),
    { clave: 'total_asis' }
  ];
...
export default ImprimirAsistenciaSesiones;
