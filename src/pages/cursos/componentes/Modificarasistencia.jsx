...
import { obtenerMoficarasist } from '../logica/Curso';
import { Form } from 'react-bootstrap';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';

function ModificarAsistencia({ datoscurso }) {
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const respuestaAsistencia = await obtenerMoficarasist(
        sede,
        semestre,
        escuela,
        curricula,
  ...
  const columnas = [
    ...
    { clave: 'nombrecompleto', titulo: 'Nombres Completos' },
    {
      ...
      titulo: 'Observación',
      render: (fila) => (
        <Form.Group controlId={`observacion-${fila.alumno}`}>
          <Form.Control
            type="text"
    ...
    },
    {
      ...
      titulo: 'Asistencia',
      render: (fila) => (
        <Form.Group controlId={`asistencia-${fila.alumno}`}>
          <Form.Select
